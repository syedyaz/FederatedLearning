...
, and Naeem Ramzan%
\thanks{Naeem Ramzan is with the University of West Scotland (e-mail: naeem.ramzan@west-scotland.ac.uk).}%
...
Extensive experimental evaluation on realistic edge datasets including FEMNIST, UCI Human Activity Recognition (HAR), and CIFAR-100 datasets with accurately modeled heterogeneous device simulations demonstrates that FedEdge-Accel achieves:
\begin{itemize}
    \item \textbf{Up to 5$\times$ faster convergence} compared to standard federated learning methods measured in communication rounds
    \item \textbf{Up to 90\% reduction in total communication cost} compared to the FedAvg baseline
    \item \textbf{Up to 80\% reduction in local training time} on resource-constrained edge devices
    \item \textbf{Comparable or superior model accuracy} (within 0.5-1\% of the full-precision baseline)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Comprehensive Empirical Evaluation}: We conduct extensive experiments on multiple diverse datasets (including federated-specific FEMNIST and sensor-based HAR) with hardware-calibrated realistic heterogeneous device simulations, demonstrating significant improvements over state-of-the-art methods.
\end{itemize}
...
To accurately evaluate FedEdge-Accel in edge computing environments, we utilize datasets that reflect real-world IoT and mobile sensing scenarios, alongside a complex vision task.

\subsubsection{FEMNIST (LEAF Benchmark)}

Federated Extended MNIST (FEMNIST) consists of 805,263 images of characters written by 3,550 distinct users. This dataset is fundamentally designed for federated learning:
\begin{itemize}
    \item \textbf{Natural Non-IID Split}: Data is naturally partitioned by the actual writer, perfectly modeling real-world system and data heterogeneity without artificial distributions.
    \item \textbf{Evaluation Focus}: Simulates a large-scale mobile user text recognition deployment.
\end{itemize}

\subsubsection{UCI Human Activity Recognition (HAR)}

The UCI HAR dataset contains 10,299 instances of 6 activities (walking, sitting, etc.) collected from smartphone accelerometers and gyroscopes.
\begin{itemize}
    \item \textbf{Federated Split}: Partitioned among 30 simulated users based on unique subject identifiers.
    \item \textbf{Evaluation Focus}: Evaluates sensor-based edge learning on highly resource-constrained IoT capabilities.
\end{itemize}

\subsubsection{CIFAR-100}

CIFAR-100 consists of 60,000 32×32 color images across 100 classes, representing a traditional computationally heavy vision task.
\begin{itemize}
    \item \textbf{Non-IID Split}: Dirichlet distribution with $\alpha = 0.5$ across 100 clients (20 classes per client on average).
\end{itemize}

\subsection{Neural Network Architectures}

We evaluate FedEdge-Accel on architectures suited for edge devices:

\begin{itemize}
    \item \textbf{CNN (FEMNIST)}: A lightweight CNN with two convolutional layers and two dense layers (approx. 1.2M parameters).
    \item \textbf{MLP (HAR)}: A 3-layer Multi-Layer Perceptron optimized for sensor data classification.
    \item \textbf{MobileNet-V2 (CIFAR-100)}: Lightweight architecture designed for mobile devices (3.4M parameters).
    \item \textbf{ResNet-18 (CIFAR-100)}: Standard architecture (11.7M parameters) for high-end device evaluation.
\end{itemize}

Models for CIFAR-100 are pre-trained and fine-tuned on target datasets before federated training.
...
\begin{itemize}
    ...
    \item \textbf{STC} \cite{sattler2019robust}: Sparse Ternary Compression framework combining structured sparsification with ternary quantization, representing a recent joint compression state-of-the-art.
\end{itemize}
...
To ensure high fidelity in our edge simulations, our hardware modeling relies on a validated discrete-event simulator calibrated against real-world edge device profilers (e.g., Android Profiler for smartphones and hardware power monitors for IoT nodes).
\begin{itemize}
    \item \textbf{Communication Model}: 
    \begin{itemize}
        \item Bandwidth: Device-specific models tracing measured metrics (5G: 100 Mbps, WiFi-6: 1 Gbps, etc.)
        \item Latency: Modeled Network RTT + simulated transmission time
        \item Energy: Calibrated radio power × transmission time ($P_{\text{radio}} = 0.5-2W$)
    \end{itemize}
    \item \textbf{Computation Model}:
    \begin{itemize}
        \item Training time: Simulated FLOPs / profiler-validated device throughput
        \item Energy: Empirical device power × training time
        \item Memory: Peak RAM footprint accounting for precision-dependent activations and model parameters
    \end{itemize}
\end{itemize}
...
\toprule
Method & Comm. Reduction (\%) & FEMNIST Acc. (\%) & HAR Acc. (\%) & CIFAR-100 Acc. (\%) \\
\midrule
FedAvg & 0 & 88.5 & 96.2 & 75.8 \\
QSGD & 60-75 & 87.1 & 94.5 & 74.9 \\
STC & 75-85 & 87.9 & 95.1 & 75.3 \\
FedEdge-Accel & \textbf{85-95} & \textbf{88.3} & \textbf{95.9} & \textbf{75.6} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Local Training Time Reduction (Profiler-Calibrated Simulation)}
...
\begin{table}[!t]
\centering
\caption{Performance Under Natural Heterogeneity (FEMNIST)}
\label{tab:heterogeneity}
\begin{tabularx}{\columnwidth}{lcc}
\toprule
Method & Target Accuracy (\%) & Comm. Rounds ($T$) \\
\midrule
FedAvg & 85.0 & 145 \\
FedProx & 85.0 & 120 \\
STC & 85.0 & 105 \\
FedEdge-Accel & \textbf{85.0} & \textbf{45} \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{Pareto Frontier of Cost Optimization}

The unified cost function ($C_{total} = \lambda_1 C_{comm} + \lambda_2 C_{comp}$) allows FedEdge-Accel to intelligently navigate the Pareto frontier between communication efficiency and computation time. By varying the $\lambda$ hyperparameters, the system can dynamically prioritize aggressive quantization (saving bandwidth) in network-congested environments, or minimize localized sparse computations (saving energy) on battery-critical devices. This trade-off capability highlights the robust adaptability of our framework to highly dynamic edge constraints.
