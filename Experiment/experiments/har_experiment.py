"""
UCI HAR Federated Learning Experiment.
"""
...
from utils.data_utils import load_har, create_iid_split, create_noniid_split, get_data_statistics, create_device_profiles
from utils.model_utils import get_har_mlp, get_model_size_mb, estimate_flops
...
def run_federated_learning(
    ...
    logger.info(f"Initializing {method_name} server for UCI HAR Experiment on device: {device}")
...
def main():
    ...
    results_dir = f"results/har_{timestamp}"
    os.makedirs(results_dir, exist_ok=True)
    
    # Setup logging
    logger = setup_logging(results_dir)
    logger.info("="*70)
    logger.info("UCI HAR Federated Learning Experiment (Publication-Ready)")
    logger.info("="*70)
    ...
    print("\n--- Phase 2: Loading HAR Dataset ---")
    data_dir = config.get('data', {}).get('data_dir', './data/har')
    
    print(f"Loading HAR from {data_dir}...")
    load_start = time.time()
    train_dataset = load_har(data_dir=data_dir, train=True)
    test_dataset = load_har(data_dir=data_dir, train=False)
    ...
    num_clients = config['datasets']['har']['num_clients']
    ...
    train_datasets_iid = create_iid_split(
        train_dataset,
        num_clients,
        config['datasets']['har']['samples_per_client_iid']
    )
    
    logger.info(f"  Creating non-IID split (alpha={config['datasets']['har']['noniid_alpha']})...")
    train_datasets_noniid = create_noniid_split(
        train_dataset,
        num_clients,
        alpha=config['datasets']['har']['noniid_alpha']
    )
    ...
    logger.info("Initializing baseline model FedAvg (MLP_HAR)...")
    baseline_model = get_har_mlp(num_classes=6)
    ...
    for scenario_name, train_datasets in scenarios:
        ...
        logger.info("-"*70)
        model_fededge = get_har_mlp(num_classes=6)
        ...
        logger.info("-"*70)
        model_fedavg = get_har_mlp(num_classes=6)
