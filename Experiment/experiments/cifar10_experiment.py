...
from utils.model_utils import get_resnet18, get_mobilenet_v2
...
def run_federated_learning(
    ...
    for round_num in tqdm(range(total_rounds), desc=f"{method_name} Training"):
        ...
        if (round_num + 1) % eval_frequency == 0:
            ...
            if not baseline:
                total_comm_cost = sum(c.get_statistics()['comm_cost'] for c in clients)
                total_comp_cost = sum(c.get_statistics()['comp_cost'] for c in clients)
...
def main():
    ...
    logger.info("="*70)
    logger.info("CIFAR-10 Federated Learning Experiment")
    logger.info("="*70)
    ...
    for scenario_name, train_datasets in scenarios:
        ...
        logger.info("-"*70)
        model_fededge = get_resnet18(num_classes=10, pretrained=True)
        ...
        logger.info("-"*70)
        model_fedavg = get_resnet18(num_classes=10, pretrained=True)
